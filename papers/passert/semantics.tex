\newcommand{\ifexpanded}[2]{\if\expandedversion1 #1 \else #2 \fi}
\ifexpanded{\includecomment{expandedonly}}{\excludecomment{expandedonly}}
\ifexpanded{\excludecomment{mainonly}}{\includecomment{mainonly}}
...
\end{mathpar}
%
\ifexpanded{Figure~\ref{sem:concrete} gives the full set of rules for the
concrete semantics.}{The full semantics appear in this paper's auxiliary
material~\cite{tr}, but we
highlight the rule for sample statements here.}
...
\ifexpanded{Figure~\ref{sem:symbolic} lists the full set of rules.}{}
...
$x_1 \leftarrow d; \; x_2 \leftarrow d; \; y := x_1 + x_2$. \ifexpanded{}{This
...
\ifexpanded{}{In our implementation, \tool proves simple cases, when an
...
$\Sigma$ but no heap.
\ifexpanded{Figure~\ref{sem:simple} shows the full set of rules. As an
example,}{The full set of rules are in the auxiliary material~\cite{tr};}
...
\ifexpanded{}{Formally, the proof of this theorem proceeds by a
structural induction. Full details are in this paper's auxiliary
material~\cite{tr}.}

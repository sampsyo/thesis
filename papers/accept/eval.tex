...
Table~\ref{table:apps} lists the applications we use in this evaluation. Since
...
qosprof, npu, truffle}.
Table~\ref{table:apps} lists the metric for each program.
In one case,
...
optimizations.
Table~\ref{table:apps} shows the source code annotation density.
...
WISP~\cite{wisp-transactions} device depicted in Figure~\ref{fig:wispphoto}. The
...
\input{plots/speedup-all.tex}\vspace{1.5ex}
\caption{Main results (all optimizations)}
\label{fig:speedup-all}
\end{subfigure}
\begin{subfigure}[b]{2.05in}
\input{plots/speedup-loopperf.tex}\vspace{1.5ex}
\caption{Loop perforation}
\label{fig:speedup-loopperf}
\end{subfigure}
\begin{subfigure}[b]{1.15in}
\input{plots/speedup-desync.tex}\vspace{1.5ex}
\caption{Sync.~elision}
\label{fig:speedup-desync}
\end{subfigure}
\begin{subfigure}[b]{1.3in}
\input{plots/speedup-npu.tex}\vspace{1.5ex}
\caption{Neural acceleration}
\label{fig:speedup-npu}
\end{subfigure}
...
Figure~\ref{fig:speedup-all} plots the speedup (versus precise execution) of the
...
Figure~\ref{fig:performance} shows the speedup for relaxed versions with only
...
%
Table~\ref{table:stats} shows the range of output error rates and speedups in
...
Using \sysname's analysis log (Section~\ref{sec:feedback}), they looked for
